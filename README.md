...
설치해도 **기본은 꺼져 있습니다.** 편집 설정의 **"폰에서 음성 인식하기"**
를 켜야 씁니다.
...
| **폰에서 음성 인식하기** | 기본 꺼짐. 한국어 정확도가 낮아 짧은 쇼츠에만 권합니다 |
| **AI 로 편집하기** | 기본 꺼짐. **돈이 듭니다** (1시간 영상에 400원 안팎). 소리 크기 대신 대사 내용을 보고 고릅니다 |
...
기본은 **소리가 커진 곳**을 하이라이트로 봅니다. 신호가 되긴 하지만 이유를
모릅니다 — 게임 효과음이 큰 구간과 진짜 웃긴 구간을 구분하지 못합니다.

AI 를 켜면 **대사를 읽고** 고릅니다. 왜 골랐는지도 같이 알려 줍니다.

```
🤖 claude-opus-5 가 구간 3개 · 밈 2개를 정했습니다
   제목 후보: 로토무한테 지는 게 말이 되냐
   · 죽고 나서 어이없어하는 리액션
   · 이겼다고 소리지름
```
...
**console.anthropic.com** → 가입 → **Billing** 에서 카드 등록 후 $5 충전 →
**API Keys** → **Create Key**.

키는 **만든 그 순간에만 보입니다.** 창을 닫으면 다시 못 봅니다.

### 2. 폰에 넣기

```bash
echo "sk-ant-여기에-복사한-키" > ~/.gameedit-key
chmod 600 ~/.gameedit-key
```

`chmod 600` 은 "나 말고 아무도 못 읽음" 입니다. 꼭 같이 치세요.
키를 **채팅이나 메신저에 붙여넣지 마세요.** 한 번 새면 그 키는 버려야 합니다.
프로그램은 키를 어디에도 통째로 찍지 않습니다 (화면에는 `sk-ant-api0…mnop`).

들어갔는지 확인:

```bash
python -m gameedit doctor
```

### 3. 켜기

편집 설정 화면의 **"AI 가 대사를 읽고 고르기"** 를 켜면 됩니다.
켜면 **이 영상의 예상 금액**과 **상한선**이 같이 나옵니다.
...
| --- | ---: | ---: | ---: |
| Haiku | 16원 | 83원 | 244원 |
| Sonnet | 47원 | 248원 | 732원 |
| **Opus (기본)** | **78원** | **414원** | **1,220원** |

$5 충전하면 1시간짜리를 Opus 로 17번 편집할 수 있습니다.

### 안전장치

- **상한선** — 넘을 것 같으면 **아예 부르지 않고** 소리 기준으로 편집합니다
- **실패해도 계속** — 인터넷이 끊기든 잔액이 떨어지든 편집은 그대로 나옵니다
- **내역서** — 끝나면 실제로 얼마가 나갔는지 화면에 남습니다

### 지금 AI 가 못 하는 것

**화면을 안 봅니다.** 자막(대사 글자)만 읽습니다. 그래서 소리도 안 나고 말도
없는데 화면에서만 벌어지는 일은 못 잡습니다. 화면까지 보는 기능은 준비 중입니다.

이 기기가 그걸 견딜지는 미리 재 볼 수 있습니다.

```bash
python -m gameedit framecheck ~/storage/shared/DCIM/영상.mp4
```
...
| --- | --- |
| 소리 분석 | 2~5분 |
| **폰 음성 인식** | **1시간 영상에 1시간 40분~2시간 30분** (G99 실측) — 그래서 기본이 꺼져 있습니다 |
| 컷 편집 + 합성 | 10분 결과물에 10~25분 |

자막은 **클로바노트·유튜브 자동자막에서 받은 .srt 를 넣는 쪽**이
빠르고 정확합니다. 그러면 음성 인식 단계를 통째로 건너뜁니다.
...
빠른 인코딩 설정으로 돕니다. 자막은 **밖에서 만든 `.srt` 를 넣는 쪽**이
기본입니다. 폰에서 직접 인식하고 싶으면 whisper.cpp 를 따로 깔고
(`bash ~/gameedit/install-subtitles.sh`) 편집 설정에서 켜면 됩니다.
