...
RULES: list[tuple[str, str, dict]] = [
    ...
    # 연출
    (r"(?:자막\s*)?(?:효과|애니\w*|움직임)\s*(?:은|는|을|를)?\s*(?:빼|없|끄|말)",
     "자막 효과 빼기 (그냥 뜨고 사라짐)", {"subtitles.animation": "off"}),
    (r"(?:효과|애니\w*|움직임)\s*(?:은|는|을|를)?\s*(?:줄|약하게|조금)",
     "자막 효과 약하게 (페이드만 · 렌더가 빨라짐)", {"subtitles.animation": "light"}),
