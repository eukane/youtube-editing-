...
DEFAULTS: dict[str, Any] = {
    ...
    "render": {
        ...
        # 클립이 이 개수 이상이면 조각을 하나씩 뽑아 이어 붙인다.
        # 한 번에 붙이면 클립 수에 비례해 메모리가 늘어난다 (실측: 24초
        # 결과물이 클립 1개 95MB, 120개 455MB). 0 이면 항상 한 번에.
        "segment_threshold": 8,
        # 한 번에 뽑을 클립 개수. 작을수록 메모리가 적고 느리다.
        # 실측(720p, 클립 120개): 전부=945MB/27s, 16개=288MB/34s,
        # 8개=224MB/48s, 1개=124MB/290s
        "segment_batch": 16,
...
PROFILES: dict[str, dict] = {
    ...
    "phone": {
        ...
        # 8배속 브릿지는 결과물 1초를 만들려고 원본 8초를 디코딩한다. 25초짜리
        # 사이 구간을 이으면 3초를 뽑으려고 25초를 읽는 셈이다. 실측으로 편집
        # 단계를 켠 뒤 '결과물 1초당 읽는 원본'이 1배에서 3배로 늘었고, 그만큼
        # 오래 걸려서 안드로이드가 백그라운드 앱을 정리할 틈이 생긴다.
        # 폰에서는 짧은 사이만 잇고 나머지는 그냥 자른다.
        "editing": {"bridge_max": 8.0, "bridge_speed": 6.0, "ramp_speed": 2.5},
    },
    # 화질 우선 (시간이 오래 걸려도 됨)
    "quality": {
